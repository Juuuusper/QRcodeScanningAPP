...
import RS.main.java.com.didihe1988.rscode.coder.RSDecoder;
...
public class ScanActivity extends AppCompatActivity implements QRCodeView.Delegate {
    ...
    //解密使用
    private String dstr,decrypt1,decrypt2,decrypt3,decrypt4;
    RSDecoder rsdecoder=new RSDecoder();
    byte[] de_byte1;
    private String key_AES = "ceet";
    private String key_RS = "abcd";
    private String key_RC4 = "ijiyg";
...
//        mZXingView.startCamera(Camera.CameraInfo.CAMERA_FACING_FRONT); // 打开前置摄像头开始预览，但是并未开始识别
    ...
    public void onScanQRCodeSuccess(String result) {
        ...
        decrypt4=result.substring(0, result.length()-64);  //去掉校验码后的密文
        decrypt3=RC4.HloveyRC4(decrypt4,key_RC4);//RC4解密

        de_byte1=BytesHexStrTranslate.toBytes(decrypt3);//将字符串转为为字节数组
        decrypt2=rsdecoder.decode(de_byte1);//RS解密
        Log.i("Rsssssssssssss", "onScanQRCodeSuccess: " +decrypt2);
        //decrypt1=new AES().Dcodes(decrypt2, key_AES);//AES解密
        Log.i("AESSSSSSSSSSSS", "onScanQRCodeSuccess: " +decrypt1);

        dstr=StrBinaryTurn.BinstrToStr(decrypt2);//得到最后的字符串

        Log.i("扫描结果为：", "onScanQRCodeSuccess: " + dstr);
        setTitle("扫描结果为：" + dstr); //toolbar上显示扫描结果
...
//                break;
            case R.id.choose_qrcde_from_gallery:
                ...
                这个库来从图库中选择二维码图片，这个库不是必须的，可以通过自己的方式从图库中选择图片
                 */
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == REQUEST_CODE_CHOOSE_QRCODE_FROM_GALLERY) {
            ...
            没有处理匿名 AsyncTask 内部类导致 Activity 泄漏的问题
            开发在使用时自行处理匿名内部类导致Activity内存泄漏的问题，处理方式可参考 https://github
